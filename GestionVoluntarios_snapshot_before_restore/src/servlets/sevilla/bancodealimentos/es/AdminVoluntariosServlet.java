...
public class AdminVoluntariosServlet extends HttpServlet {
    ...
    private void handleSave(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        // Obtenemos los parÃ¡metros de forma segura
        ...
        try (Connection conn = DatabaseUtil.getConnection()) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                if (fechaNacimientoStr != null && !fechaNacimientoStr.trim().isEmpty()) {
                    try {
                        ...
                        stmt.setNull(6, Types.DATE); // Si el formato es invÃ¡lido, guarda NULL
                    }
                } else {
                    stmt.setNull(6, Types.DATE); // Si estÃ¡ vacÃ­o, guarda NULL
                }
                
                stmt.setString(7, cp);

                // --- MANEJO ROBUSTO DE NÃšMERO (TIENDA) ---
                if (tiendaReferenciaStr != null && !tiendaReferenciaStr.trim().isEmpty()) {
                    try {
                        stmt.setInt(8, Integer.parseInt(tiendaReferenciaStr));
                    } catch (NumberFormatException e) {
                        stmt.setNull(8, Types.INTEGER); // Si no es un nÃºmero vÃ¡lido, guarda NULL
                    }
                } else {
                    stmt.setNull(8, Types.INTEGER); // Si estÃ¡ vacÃ­o, guarda NULL
                }
            ...
            String logComment = "Admin " + adminUser + " modificÃ³ los datos del voluntario: " + usuarioToUpdate;
            LogUtil.logOperation(conn, "ADMIN_UPDATE_VOL", adminUser, logComment);
            
            sqlRowUuid = getSqlRowUuid(conn, usuarioToUpdate);
            conn.commit();
            
            // ReplicaciÃ³n a SharePoint (si procede)
            if (sqlRowUuid != null) {
                try {
                    ...
                    if (tiendaReferenciaStr != null && !tiendaReferenciaStr.isEmpty()) {
                        ...
                        } catch (NumberFormatException e) { /* No se envÃ­a si no es un nÃºmero */ }
                    }
                    ...
                    System.err.println("ADVERTENCIA: Fallo al replicar a SharePoint la modificaciÃ³n de datos para " + usuarioToUpdate + ". Causa: " + e.getMessage());
                }
            }
            ...
            e.printStackTrace();
            sendError(response, "Error al actualizar los datos. El DNI o el email podrÃ­an estar ya en uso.");
        } catch (Exception e) { // Captura genÃ©rica para cualquier otro error inesperado
            e.printStackTrace();
    ...
    private void handleToggleAdmin(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (usuarioToUpdate == null || newAdminStatus == null || (!"S".equals(newAdminStatus) && !"N".equals(newAdminStatus))) {
            sendError(response, "Datos invÃ¡lidos para la actualizaciÃ³n.");
            return;
        }
        ...
        try (Connection conn = DatabaseUtil.getConnection()) {
            ...
            String logComment = "Admin " + adminUser + " cambiÃ³ el rol de " + usuarioToUpdate + " a admin=" + newAdminStatus;

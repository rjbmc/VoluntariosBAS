...
public class ModificarDatosServlet extends HttpServlet {
        ...
        try {
            ...
            if (nuevaClave != null && !nuevaClave.isEmpty()) {
                if (!verificarYCambiarClave(conn, usuario, claveActual, nuevaClave)) {
                    ...
                    jsonResponse.addProperty("message", "La contraseÃ±a actual no es correcta.");
            ...
            if (emailHaCambiado) {
                ...
                jsonResponse.addProperty("message", "Â¡Datos guardados! Se ha enviado un correo a tu nueva direcciÃ³n para verificar el cambio.");
            } else {
                jsonResponse.addProperty("success", true);
                jsonResponse.addProperty("message", "Â¡Datos actualizados correctamente!");
            }
            ...
            if (sqlRowUuid != null) {
                try {
                    ...
                    // ** CORRECCIÃ“N: AÃ±adido el Site ID como segundo parÃ¡metro **
                    ...
                    System.err.println("ADVERTENCIA: Fallo al iniciar el proceso de replicaciÃ³n a SharePoint para el UUID: " + sqlRowUuid + ". Causa: " + e.getMessage());
                }
            }
    ...
    private boolean verificarYCambiarClave(Connection conn, String usuario, String claveActual, String nuevaClave) throws SQLException {
        ...
        if (PasswordUtils.checkPassword(claveActual, claveGuardada)) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement("UPDATE voluntarios SET Clave = ? WHERE Usuario = ?")) {
                ...
                stmt.executeUpdate();
                LogUtil.logOperation(conn, "MODIF_PASS", usuario, "ContraseÃ±a actualizada por el propio usuario.");
                return true;
            }
        }
        return false;
    ...
    private void sendEmailConfirmacionCambio(String emailDestino, String token, String usuario) {
        ...
        String htmlContent = "Por favor, haz clic en el siguiente enlace para confirmar tu nueva direcciÃ³n de correo: " + link;
        ...
        try {
			...
			message.setSubject("Confirma tu nueva direcciÃ³n de correo - Voluntarios Banco de Alimentos de Sevilla");
			message.setContent(htmlContent, "text/html; charset=utf-8");
			Transport.send(message);
		} catch (MessagingException e) {
			 try {
				LogUtil.logOperation("CHANGE_EMAIL_CONF", usuario, "Error en envio de la confirmaciÃ³n del cambio de email a " + emailDestino);

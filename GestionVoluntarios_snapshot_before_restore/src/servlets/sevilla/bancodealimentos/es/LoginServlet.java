            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        // 1. Comprobar si la cuenta estÃ¡ dada de baja
                        if (fechaBaja != null) {
                        ...
                        // 2. Comprobar la contraseÃ±a
                        else if (storedHash != null && BCrypt.checkpw(clave, storedHash)) {
                            // 3. Comprobar si la cuenta estÃ¡ verificada
                            if ("S".equals(verificado)) {
                                ...
                                session.setAttribute("email", email); // Guardamos el email en la sesiÃ³n
                                ...
                                LogUtil.logOperation("LOGIN-OK", usuario, "Inicio de sesiÃ³n exitoso.");

                            } else {
                                jsonResponse.addProperty("success", false);
                                jsonResponse.addProperty("message", "Tu cuenta aÃºn no ha sido verificada. Por favor, revisa tu correo electrÃ³nico.");
                                LogUtil.logOperation("LOGIN-FAIL", usuario, "Intento de login sin verificar email.");
                            }
                        } else {
                            jsonResponse.addProperty("success", false);
                            jsonResponse.addProperty("message", "Usuario o contraseÃ±a incorrectos.");
                            LogUtil.logOperation("LOGIN-FAIL", usuario, "ContraseÃ±a incorrecta.");
                        }
                    } else {
                        jsonResponse.addProperty("success", false);
                        jsonResponse.addProperty("message", "Usuario o contraseÃ±a incorrectos.");

...
public class CleanupTiendasServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (session == null || session.getAttribute("usuario") == null || !Boolean.TRUE.equals(session.getAttribute("isAdmin"))) {
            out.println("ERROR: Acceso denegado. No tienes permisos para realizar esta operaciÃ³n.");
            out.flush();
        ...
        try {
            out.println("--- INICIANDO LIMPIEZA PROFUNDA (MODO DIAGNÃ“STICO) ---");
            out.flush();
            ...
            if (listId == null) {
                throw new Exception("ERROR CRÃTICO: La lista '" + SHAREPOINT_LIST_NAME + "' no fue encontrada en SharePoint.");
            }
            ...
            out.println("Paso 2/4: Iniciando bucle de eliminaciÃ³n. Esto puede tardar muchos minutos...");
            out.flush();
            ...
            while (true) {
                ...
                if (currentPage == null || currentPage.getValue().isEmpty()) {
                    out.println("[Pase " + pases + "] Respuesta vacÃ­a. No se encontraron mÃ¡s items.");
                    out.flush();
                ...
                for (int i = 0; i < itemsEnPagina.size(); i++) {
                    ...
                    if ((i + 1) % 100 == 0) { // AÃ±ade un salto de lÃ­nea cada 100 puntos
                        out.println();
                    }
                    out.flush();
            ...
            out.println("\n----------------------------------------------------------");
            out.println("Paso 3/4: VerificaciÃ³n final. Comprobando si la lista estÃ¡ realmente vacÃ­a...");
            out.flush();
            ...
            if(finalCheck != null && !finalCheck.getValue().isEmpty()) {
                 throw new Exception("Â¡FALLO DE VERIFICACIÃ“N! La limpieza terminÃ³, pero la lista todavÃ­a contiene " + finalCheck.getValue().size() + " items.");
            }
            out.println("VerificaciÃ³n final exitosa. La lista estÃ¡ vacÃ­a.");
            out.flush();
            
            String successMessage = "Â¡Ã‰XITO! Limpieza profunda completada. Se han eliminado un total de " + totalEliminados + " items.";
            out.println("\n----------------------------------------------------------");
            out.println("Paso 4/4: " + successMessage);
            out.flush();

        } catch (Exception e) {
            out.println("\n\n--- Â¡ERROR CRÃTICO DURANTE LA LIMPIEZA! ---");

...
public class SolicitarBajaServlet extends HttpServlet {
    private static final long serialVersionUID = 4L; // VersiÃ³n actualizada
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (session == null || session.getAttribute("usuario") == null) {
            ...
            jsonResponse.addProperty("message", "No hay una sesiÃ³n de usuario activa. Por favor, inicia sesiÃ³n de nuevo.");
        ...
        try (Connection conn = DatabaseUtil.getConnection()) {
            ...
            if (hashedPassword == null || !BCrypt.checkpw(plainPassword, hashedPassword)) {
                ...
                jsonResponse.addProperty("message", "La contraseÃ±a es incorrecta.");
                response.getWriter().write(jsonResponse.toString());
                LogUtil.logOperation(conn, "SOLICITUD_BAJA_FAIL", usuario, "Intento de baja con contraseÃ±a incorrecta.");
                conn.rollback();
                return;
            }
            ...
            // 1. Invalidar la sesiÃ³n actual para cerrar sesiÃ³n
            session.invalidate();

            // 2. Preparar la respuesta JSON con la instrucciÃ³n de redirecciÃ³n
            jsonResponse.addProperty("success", true);
            jsonResponse.addProperty("message", "Tu solicitud de baja ha sido procesada correctamente. SerÃ¡s redirigido a la pÃ¡gina de inicio.");

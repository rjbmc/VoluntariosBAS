...
public class SyncAsignacionesServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (session == null || session.getAttribute("usuario") == null || !Boolean.TRUE.equals(session.getAttribute("isAdmin"))) {
            ...
            jsonResponse.addProperty("message", "Acceso denegado. La sesiÃ³n ha expirado. Por favor, cierra sesiÃ³n y vuelve a entrar.");
        ...
        try (Connection conn = DatabaseUtil.getConnection()) {
            ...
            jsonResponse.addProperty("message", "SincronizaciÃ³n de Asignaciones completada. Total: " + totalProcesados + ". Fallidos: " + fallidos + ". Errores: " + (errorLog.length() > 0 ? errorLog.toString() : "Ninguno"));
            
        } catch (Exception e) {
            e.printStackTrace();
            jsonResponse.addProperty("success", false);
            jsonResponse.addProperty("message", "Error CRÃTICO en SyncAsignacionesServlet: " + e.getMessage());
        } finally {
            response.getWriter().write(jsonResponse.toString());
        }
    }

...
public class NuevoVoluntarioServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (Period.between(fechaNacimiento, LocalDate.now()).getYears() < 16) {
                ...
                jsonResponse.addProperty("message", "Debes tener al menos 16 aÃ±os para poder registrarte.");
                response.getWriter().write(jsonResponse.toString());
                return;
            }
        } catch (java.time.format.DateTimeParseException e) {
            jsonResponse.addProperty("success", false);
            jsonResponse.addProperty("message", "El formato de la fecha de nacimiento no es vÃ¡lido.");
            response.getWriter().write(jsonResponse.toString());
            return;
        }
        ...
        try {
            ...
            if (existingUser != null && !isInactive && isVerified) {
                jsonResponse.addProperty("success", false);
                jsonResponse.addProperty("message", "El nombre de usuario o el DNI ya estÃ¡n registrados en una cuenta activa y verificada.");
                conn.rollback();
                response.getWriter().write(jsonResponse.toString());
                return;
            }
            ...
            if (existingUser != null && !isInactive && !isVerified) {
                // ... (lÃ³gica para reenviar verificaciÃ³n sin cambios)
            }

            if (existingUser != null && isInactive) {
                isReactivation = true;
                // ... (lÃ³gica de update local sin cambios)

            } else {
                sqlRowUuid = UUID.randomUUID().toString(); // Usamos el mÃ©todo nativo
                // ... (lÃ³gica de insert local sin cambios)
            }

            conn.commit();

            // *** COMIENZA LA NUEVA LÃ“GICA DE REPLICACIÃ“N A SHAREPOINT ***
            try {
                ...
                spData.put("TiendaReferenciaId", tiendaReferencia); // Asumiendo que el campo lookup se llama asÃ­
                ...
                LogUtil.logOperation(conn, "SP_REPLICATION_ERROR", "SYSTEM", "Fallo en replicaciÃ³n para UUID: " + sqlRowUuid + ". Causa: " + e.getMessage());
                e.printStackTrace();
            }
            // *** FIN DE LA NUEVA LÃ“GICA DE REPLICACIÃ“N ***

            sendVerificationEmail(request, email, usuario, verificationToken);

            jsonResponse.addProperty("success", true);
            jsonResponse.addProperty("message", "Â¡Registro casi completo! Se ha enviado un correo de verificaciÃ³n a tu email. Por favor, revÃ­salo para activar tu cuenta.");
    ...
    private void sendVerificationEmail(HttpServletRequest request, String emailDestino, String usuario, String token){
        // ... (cÃ³digo de envÃ­o de email sin cambios)
    }
}

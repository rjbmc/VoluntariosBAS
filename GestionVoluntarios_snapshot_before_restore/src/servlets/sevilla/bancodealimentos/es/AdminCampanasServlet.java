...
public class AdminCampanasServlet extends HttpServlet {
    private static final long serialVersionUID = 5L; // VersiÃ³n actualizada
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection conn = DatabaseUtil.getConnection();
            ...
            throw new ServletException("Error de base de datos al obtener campaÃ±as", e);
        }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (action == null) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST, "AcciÃ³n no especificada.");
            return;
        }
        ...
        switch (action) {
            ...
            case "deactivate":
                ...
                break;
            default:
                response.sendError(HttpServletResponse.SC_BAD_REQUEST, "AcciÃ³n desconocida.");
        }
    }
    ...
    private void handleSave(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try (Connection conn = DatabaseUtil.getConnection()) {
            ...
            try {
                ...
                LogUtil.logOperation(conn, "SP_CAMP_SAVE_ERROR", adminUser, "Error replicando guardado de campaÃ±a " + campanaId + ": " + e.getMessage());
                e.printStackTrace();
            }

            String logComment = isUpdate ? "Modificada campaÃ±a: " : "Creada campaÃ±a: ";
            LogUtil.logOperation(conn, "ADMIN_SAVE_CAMP", adminUser, logComment + campanaId);
            conn.commit();
            sendJsonResponse(response, true, "CampaÃ±a guardada correctamente.");

        } catch (Exception e) {
            e.printStackTrace();
            sendJsonResponse(response, false, "Error al guardar la campaÃ±a: " + e.getMessage());
        }
    }
            ...
            try {
                ...
                if (itemId != null) {
                    SharepointUtil.deleteListItem(SharepointUtil.SITE_ID, listId, itemId);
                     ...
                     LogUtil.logOperation(conn, "SP_CAMP_DEL_WARN", adminUser, "No se encontrÃ³ item en SP para borrar campaÃ±a " + campanaId);
                }
            } catch (Exception e) {
                LogUtil.logOperation(conn, "SP_CAMP_DEL_ERROR", adminUser, "Error replicando borrado de campaÃ±a " + campanaId + ": " + e.getMessage());
                e.printStackTrace();
            }
            
            LogUtil.logOperation(conn, "ADMIN_DELETE_CAMP", adminUser, "Eliminada campaÃ±a: " + campanaId);
            conn.commit();
            sendJsonResponse(response, true, "CampaÃ±a eliminada correctamente.");

        } catch (Exception e) {
            e.printStackTrace();
            sendJsonResponse(response, false, "Error al eliminar la campaÃ±a. AsegÃºrate de que no tenga voluntarios asignados.");
        }
    }
        ...
        try (Connection conn = DatabaseUtil.getConnection()) {
            ...
            try {
                ...
                if (itemId != null) {
                    ...
                    LogUtil.logOperation(conn, "SP_CAMP_ACT_WARN", adminUser, "No se encontrÃ³ item en SP para activar campaÃ±a " + campanaId);
                }
            } catch (Exception e) {
                LogUtil.logOperation(conn, "SP_CAMP_ACT_ERROR", adminUser, "Error replicando activaciÃ³n de campaÃ±a " + campanaId + ": " + e.getMessage());
                e.printStackTrace();
            }

            LogUtil.logOperation(conn, "ADMIN_ACTIVATE_CAMP", adminUser, "Activada campaÃ±a: " + campanaId);
            conn.commit();
            sendJsonResponse(response, true, "CampaÃ±a activada correctamente.");

        } catch (Exception e) {
            e.printStackTrace();
            sendJsonResponse(response, false, "Error al activar la campaÃ±a.");
        }
    }
        ...
        try (Connection conn = DatabaseUtil.getConnection()) {
            ...
            try {
                ...
                if (itemId != null) {
                    ...
                    LogUtil.logOperation(conn, "SP_CAMP_DEACT_WARN", adminUser, "No se encontrÃ³ item en SP para desactivar campaÃ±a " + campanaId);
                }
            } catch (Exception e) {
                LogUtil.logOperation(conn, "SP_CAMP_DEACT_ERROR", adminUser, "Error replicando desactivaciÃ³n de campaÃ±a " + campanaId + ": " + e.getMessage());
                e.printStackTrace();
            }
            
            LogUtil.logOperation(conn, "ADMIN_DEACTIVATE_CAMP", adminUser, "Desactivada campaÃ±a: " + campanaId);
            conn.commit();
            sendJsonResponse(response, true, "CampaÃ±a desactivada correctamente.");

        } catch (Exception e) {
            e.printStackTrace();
            sendJsonResponse(response, false, "Error al desactivar la campaÃ±a.");
        }
    }

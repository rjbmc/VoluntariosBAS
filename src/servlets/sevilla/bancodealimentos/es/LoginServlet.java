...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            logger.error("Error SQL durante el login", e);
            sendJsonResponse(response, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, false, "Error de base de datos: " + e.getMessage());
        } catch (Exception e) {
            LogUtil.logException(logger, e, "Error inesperado durante el login", context);
            sendJsonResponse(response, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, false, "Error interno del servidor.");
        } finally {
            if (conn != null) try { conn.close(); } catch (SQLException e) { 
                LogUtil.logException(logger, e, "Error cerrando conexión en LoginServlet", context); 
            }
        }
    }

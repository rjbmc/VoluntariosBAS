...
public class GuardarTurnosServlet extends HttpServlet {
    ...
    /**
     * Replica la asignación a la lista de SharePoint.
     * MODIFICADO: Ahora siempre actualiza el elemento si existe, en lugar de borrarlo cuando no hay turnos.
     */
    private void replicarAsignacionASharePoint(Connection conn, String campanaId, String usuario) throws Exception {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sqlSelect)) {
            ...
            if (rs.next()) {
                for (int i = 1; i <= 4; i++) {
                    ...
                    // --- Valores por defecto (limpian el turno en SharePoint) ---
                    fields.getAdditionalData().put("Turno" + i + "LookupId", null);
                    fields.getAdditionalData().put("Comentario" + i, comentario != null ? comentario : "");
                    fields.getAdditionalData().put("Turno" + i + "Codigo", ""); // Si el campo existe en SP
                    ...
                    if (idTiendaDb > 0) {
                        ...
                        if (tiendaUuid != null) {
                            // Buscar en SharePoint por SqlRowUUID
        ...
        // --- MODIFICACIÓN: Conservar siempre el registro en SharePoint, actualizándolo si existe ---
        if (itemId != null) {
            // Siempre actualizamos (tanto si tiene turnos como si no)
            SharePointUtil.updateListItem(SharePointUtil.SITE_ID, listIdAsignaciones, itemId, fields);
        } else if (tieneTurnos) {
            // Solo creamos si hay al menos un turno (para no generar registros vacíos innecesarios)
            SharePointUtil.createListItem(SharePointUtil.SITE_ID, listIdAsignaciones, fields);
        }
        // Si no tiene turnos y no existía previamente, no hacemos nada (coherente con la BD local)
    }
